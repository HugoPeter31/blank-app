...
# - HSG green table headers (global CSS)
# - Sidebar "About" section
# - Toast notifications for non-critical confirmations
# - Issue search (free-text) + better status readability (icons)
# - SLA overdue flag + metric
# - Booking conflict preview before submission
# - Wider layout for dashboards/tables
# ============================================================================
...
# HSG brand green for consistent UI styling
HSG_GREEN = "#00802F"
...
class Submission:
    """Represents a validated issue submission from the reporting form."""
    name: str
    hsg_email: str
    issue_type: str
    room_number: str
...
def get_secret(key: str, default: str | None = None) -> str:
    """Retrieve Streamlit secret safely; stop app if required secret is missing."""
...
def normalize_room(room_number: str) -> str:
    """Normalize inputs like 'A09-001' to canonical format 'A 09-001'."""
...
def get_connection() -> sqlite3.Connection:
    """Cache connection to avoid re-opening DB on every Streamlit rerun."""
...
def apply_hsg_table_header_style() -> None:
    """Apply HSG green styling to all Streamlit table headers."""
    st.markdown(
        f"""
        <style>
...
def show_logo() -> None:
    try:
        ...
        st.sidebar.warning("Logo not found. Add 'HSG-logo-new.png' to the repository root.")


def sidebar_about() -> None:
    """Central place for quick user guidance without cluttering the main pages."""
    with st.sidebar.expander("ℹ️ About this app", expanded=False):
        st.markdown(
            """
            **Reporting Tool @ HSG**
            - Report facility issues with SLA tracking  
            - Book rooms & assets (room booking blocks included items)  
            - Admin panel manages assignments and statuses  
            """
        )
...
def page_submission_form(con: sqlite3.Connection) -> None:
    ...
    st.toast("✅ Submission received.", icon="✅")  # UI improvement: lightweight feedback
    if not ok:
        st.warning(msg)
...
def build_display_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # UI improvement: make status/importance more scannable
    display_df["STATUS"] = display_df["STATUS"].map(lambda s: STATUS_DISPLAY.get(str(s), str(s)))
    display_df["IMPORTANCE"] = display_df["IMPORTANCE"].map(lambda s: IMPORTANCE_DISPLAY.get(str(s), str(s)))

    importance_order = {"High": 0, "Medium": 1, "Low": 2}
    display_df["_imp_rank"] = (
        df["importance"].map(importance_order).fillna(99).astype(int)
    ...
    ).drop(columns=["_imp_rank"], errors="ignore")
...
def page_submitted_issues(con: sqlite3.Connection) -> None:
    ...
    # UI improvement: quick search across visible issues (doesn't replace filters)
    search_text = st.text_input("Search in issues", placeholder="e.g., A 09-001, network, projector…").strip().lower()
    ...
    # Compute SLA target once and reuse (avoid double-calling expected_resolution_dt)
    ...
    # UI improvement: overdue flag to highlight SLA breaches
    now_dt = now_zurich()
    df_view["sla_dt"] = pd.to_datetime(df_view["expected_resolved_at"], errors="coerce")
    df_view["overdue"] = (
        (df_view["status"] != "Resolved")
        & df_view["sla_dt"].notna()
        & (df_view["sla_dt"] < now_dt)
    )

    # Apply search after computed fields so users can search SLA/assignee too
    if search_text:
        mask = df_view.apply(
            lambda r: search_text in " ".join(map(str, r.values)).lower(),
            axis=1,
        )
        df_view = df_view[mask].copy()

    if df_view.empty:
        st.info("No issues match your search / filter combination.")
        return
    ...
    with c2:
        st.metric("Overdue (SLA breached)", str(int(df_view["overdue"].sum())))

    display_df = build_display_table(df_view)
    st.subheader("List of submitted issues")
    st.dataframe(display_df, use_container_width=True, hide_index=True)

    csv_bytes = df_view.drop(columns=["sla_dt"], errors="ignore").to_csv(index=False).encode("utf-8")
...
def page_booking(con: sqlite3.Connection) -> None:
    ...
    with st.form("booking_form"):
        ...
        # UI improvement: conflict preview before submit (reduces frustration)
        ...
        if user_name and start_dt_preview >= now_zurich():
            if not is_asset_available(con, asset_id, start_dt_preview, end_dt_preview):
                st.warning("⛔ This time slot conflicts with an existing booking.")
    ...
    st.toast("✅ Booking confirmed.", icon="✅")  # UI improvement
    st.rerun()
...
def page_assets(con: sqlite3.Connection) -> None:
    ...
    if st.button("Update location"):
        ...
        st.toast("✅ Location updated.", icon="✅")  # UI improvement
        st.rerun()
...
def page_overwrite_status(con: sqlite3.Connection) -> None:
    ...
    if c1.button("Send weekly report now"):
        ...
        if ok:
            ...
            st.toast("✅ Weekly report sent.", icon="✅")
        else:
            st.warning(msg)
    ...
    if st.button("Update"):
        ...
        st.toast("✅ Issue updated.", icon="✅")  # UI improvement
        st.rerun()
...
# ============================================================================
def main() -> None:
    # UI improvement: wider layout for dashboards/tables
    st.set_page_config(page_title="Reporting Tool @ HSG", layout="wide")
